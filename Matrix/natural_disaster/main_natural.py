...
class NaturalDisasterGame:
    ...
    def play_lava(self):
        """Jocul 1: The Floor is Lava (Supraviețuire și Poziționare)"""
        ...
        total_frames = int((120 + (self.num_players * 20)) * diff_mult)
        ...
        for f in range(total_frames):
            ...
            # 2. Verifică jucătorii călcați de lavă
    ...
    def play_meteors(self):
        """Jocul 2: Meteor Shower (Reacție și Evitare)"""
        self.ui.update_dashboard("METEORIȚI!", "Evită zonele ROȘII!", "red")
        diff_mult = self.get_difficulty_multiplier()
        
        spawn_rate = max(3, int((4 + self.num_players) * diff_mult)) 
        meteor_timer = max(10, int((25 + self.num_players * 3) * diff_mult))
        ...
        for f in range(250):
            ...
            # 1. Gestionarea craterelor vechi
            for c in self.craters[:]:
                for dx in [-1, 0, 1]:
                    for dy in [-1, 0, 1]: 
                        self.hw.set_pixel_physical(frame, c['x']+dx, c['y']+dy, COLORS["CRATER"])
                c['life'] -= 1
                if c['life'] <= 0: self.craters.remove(c)
                
            # 2. Generare meteoriți noi
            if f % spawn_rate == 0: 
                self.meteors.append({'x': random.randint(1, WIDTH-2), 'y': random.randint(1, HEIGHT-2), 'timer': meteor_timer})
                
            # 3. Logica de impact a meteoriților
            for m in self.meteors[:]:
                mx, my, timer = m['x'], m['y'], m['timer']
                
                if timer > 8: 
                    # Faza de avertizare (Pâlpâie roșu)
                    if f % 3 == 0: 
                        for dx in [-1, 0, 1]:
                            for dy in [-1, 0, 1]:
                    ...
                    # Faza de impact (Culoare plină și daună)
                    for dx in [-1, 0, 1]:
                    ...
                    # Verificăm dacă cineva a stat în raza exploziei
                    for px, py in self.hw.pressed_buttons:
                        if (px, py) in self.splashing_positions: continue
    ...
    def play_fire(self):
        """Jocul 3: Firefighter (Cooperare și Mișcare rapidă)"""
        ...
        spread_interval = max(2, int((3 + self.num_players) * diff_mult)) 
        spread_chance = max(0.05, (0.12 - (self.num_players * 0.01)) / diff_mult) 
        max_fire_pixels = int((WIDTH * HEIGHT) * 0.45) # Dacă 45% e foc, se pierde runda
        round_survived = True
        ...
        for f in range(250):
            ...
            # Menținem minim un foc aprins constant
            if len(self.fire_pixels) == 0:
                for _ in range(max(2, self.num_players)):
                    self.fire_pixels.add((random.randint(1, WIDTH-2), random.randint(1, HEIGHT-2)))

            # Logica de propagare a focului
            ...
            # Stingerea focului prin călcare
            ...
            # Condiția de înfrângere a rundei (foc prea mare)
