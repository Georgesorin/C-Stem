...
def _load_config():
    ...
    # SCHIMBARE: Simulatorul trebuie să ASCULTE pe 4626 și să TRIMITĂ pe 7800
    defaults = {"send_port": 7800, "recv_port": 4626, "device_ip": "127.0.0.1"}
    if os.path.exists(cfg_path):
        try:
            with open(cfg_path, "r") as f:
                cfg = json.load(f)
                # Inversăm rolurile față de config-ul jocului
                return {
                    "send_port": cfg.get("recv_port", 7800), 
                    "recv_port": cfg.get("send_port", 4626), 
...
class SandrunSimulator:
    def __init__(self, root):
        self.root = root
        self.root.title(f"Matrix Simulator - Listening on: {CONFIG['recv_port']}")
        ...
        try:
            ...
            print(f"[!] Eroare: Portul {CONFIG['recv_port']} este ocupat. Închide alte instanțe.")
            root.destroy()
            return
    ...
    def network_loop(self):
        while self.running:
            try:
                ...
                # Logică procesare pachete 0x75 (Matrix)...
                if data[0] == 0x75 and len(data) >= 12:
                    cmd = struct.unpack(">H", data[8:10])[0]
                    if cmd == 0x8877:
                        pkt_idx = struct.unpack(">H", data[10:12])[0]
                        if pkt_idx == 0xFFF0: continue 
                        chunk = data[14:-2]
                        off = (pkt_idx - 1) * 984
                        if off + len(chunk) <= len(self.frame_buffer):
    ...
    def update_display(self):
        for led_pos in range(LEDS_PER_CHANNEL):
            for ch in range(NUM_CHANNELS):
                off = led_pos * 24 + ch
                if off + 16 < len(self.frame_buffer):
    ...
    def send_input(self):
        ...
        for ch, led in self.pressed_leds: 
            if 0 <= ch < 8 and 0 <= led < 64:
                pkt[2 + ch * 171 + 1 + led] = 0xCC
