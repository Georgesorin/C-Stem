...
class DashboardUI:
    ...
    def _build_setup_frame(self):
        ...
        btn_fire = tk.Label(games_frame, text="FIREFIGHTER", font=("Consolas", 30, "bold"), bg="#FF8C00", fg="white", pady=10, cursor="hand2")
        btn_fire.bind("<Button-1>", lambda e: self._show_tutorial("fire"))
        btn_fire.pack(pady=10, fill="x")
    ...
    def _show_tutorial(self, game_mode):
        ...
        if game_mode == "lava":
            ...
            self.lbl_tut_title.config(text="FIREFIGHTER", fg="#FF8C00")
            self.lbl_tut_desc.config(text="SCOP: Salvați pădurea de incendiu!\n\n1. Focul (PORTOCALIU/GALBEN) se răspândește rapid.\n2. Călcați pe fiecare flacără pentru a o stinge.\n3. Dacă focul scapă de sub control (>45%), pierdeți o viață.\n\nLucrați în echipă și stingeți totul pentru a avansa!")
    ...
    def _build_game_frame(self):
        ...
        # BUTONUL NOU - Creat, dar neafișat încă (îl ascundem)
        ...
        self.lbl_score = tk.Label(self.game_frame, text="RUNDE SUPRAVIEȚUITE: 0", font=("Consolas", 45, "bold"), bg="black", fg="#00FF00")
    ...
    def _start_game(self):
        ...
        # Ascundem butonul la începutul unui joc nou, în caz că a rămas de la runda trecută
        self.btn_return.pack_forget() 
        
        # Trimitem toate setările către motorul de joc!
    ...
    def update_score(self, score):
        self.root.after(0, lambda: self.lbl_score.config(text=f"RUNDE: {score}"))
    ...
    def show_game_over(self, score):
        ...
        self.root.after(0, lambda: self.lbl_score.config(text=f"TOTAL RUNDE: {score}", fg="yellow"))
        self.root.after(0, lambda: self.lbl_lives.config(text="💀💀💀"))
        
        # Afișăm butonul de întoarcere la meniu, sub statusul de game over
